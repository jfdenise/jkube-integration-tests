...
class WildflyOcITCase extends Wildfly {
  ...
  @DisplayName("oc:build, should create image")
    ...
    final ImageStream is = oc.imageStreams().withName("wildfly-microprofile").get();
  ...
  void ocResource() throws Exception {
    ...
    assertThat(new File(metaInfDirectory, "jkube/openshift/wildfly-microprofile-deploymentconfig.yml"), yaml(not(anEmptyMap())));
    assertThat(new File(metaInfDirectory, "jkube/openshift/wildfly-microprofile-route.yml"), yaml(not(anEmptyMap())));
    assertThat(new File(metaInfDirectory, "jkube/openshift/wildfly-microprofile-service.yml"), yaml(not(anEmptyMap())));
  }
