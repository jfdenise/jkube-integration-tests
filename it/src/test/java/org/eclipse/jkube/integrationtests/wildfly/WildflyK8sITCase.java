 ...
 */
package org.eclipse.jkube.integrationtests.wildfly;
...
class WildflyK8sITCase extends Wildfly {
  ...
  @DisplayName("k8s:build, should create image")
    ...
    assertImageWasRecentlyBuilt("integration-tests", "wildfly-microprofile");
  }
  ...
  void k8sResource() throws Exception {
    ...
    assertThat(new File(metaInfDirectory, "jkube/kubernetes/wildfly-microprofile-deployment.yml"), yaml(not(anEmptyMap())));
    assertThat(new File(metaInfDirectory, "jkube/kubernetes/wildfly-microprofile-service.yml"), yaml(not(anEmptyMap())));
  }
  ...
  void k8sApply() throws Exception {
    ...
    awaitDeployment(this, pod.getMetadata().getNamespace())
      ...
      .assertContainers(hasItems(allOf(
        hasProperty("image", equalTo("integration-tests/wildfly-microprofile:latest")),
        hasProperty("name", equalTo("wildfly")),
        ...
        hasProperty("env", hasItems(allOf(
          hasProperty("name", equalTo("SERVER_ARGS")),
